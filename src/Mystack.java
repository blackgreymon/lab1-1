public class Mystack { //存储多变量
    ...
    public Boolean add(String str) { //拆分函数，将表达式拆分成单项式数组，继而拆分成多变量信息
    ...
    private int toNum(String str) { //转数字
        int tmp = 0;
        int flag = 1;
        char operator = str.charAt(0);
        if (operator == '-') {
    ...
    private Boolean isNum(String str) { //判断数字
        char operator = str.charAt(0);
        if (operator != '+' && operator != '-' && (operator < '0' || operator > '9')) {
			return false;
		}
        for (int i = 1; i < str.length(); i++) {
    ...
    private Boolean isVar(String str) { //判断变量
    ...
    public void simplify(String str) { //化简函数
    ...
    public void derivative(String str) { //求导函数
        String vars[] = new String[100];
        int top = 0;
        vars[top++] = str;
        String ans = "";
